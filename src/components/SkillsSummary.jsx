      ...
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {Object.entries(skills).map(([category, skillsList], index) => (
            <div
              key={index}
              className="p-4 border rounded-lg shadow-lg bg-white dark:bg-gray-900"
            >
              ...
              <ul className="space-y-4">
                {skillsList.map((skill, i) => (
                  <li
                    key={i}
                    className="flex items-center space-x-3 text-md text-center"
                  >
                    <span className="text-3xl text-gray-900">{skill.icon}</span>
